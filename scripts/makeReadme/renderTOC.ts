...
const renderClassTOC = (classObj: DeclarationReflection): string => {
    ...
    const properties = classObj.children?.filter(child => child.kindString === 'Accessor' && !child.flags.includes('Private')).sort(comparator) || [];
    ...
    const methods = classObj.children?.filter(child => child.kindString === 'Method' && !child.flags.includes('Private')).sort(comparator) || [];
    if (methods.length) {
